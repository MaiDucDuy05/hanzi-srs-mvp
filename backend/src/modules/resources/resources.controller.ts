...
export class ContactController {
  ...
  @Public()
  @Post() @HttpCode(HttpStatus.CREATED) async create(@Body() dto: DTO.CreateContactRequestDto) { return ok(await this.svc.create(dto), 'Contact request sent'); }
...
export class VipUpgradeController {
  ...
  @Post() @HttpCode(HttpStatus.CREATED) async create(@Body() dto: DTO.CreateVipUpgradeRequestDto) { return ok(await this.svc.create(dto), 'VIP upgrade request created'); }
  @Patch(':id') @Roles(Role.ADMIN) async review(@Param('id') id: string, @Body() dto: DTO.ReviewVipUpgradeDto, @CurrentUser('sub') reviewedBy: string) { return ok(await this.svc.review(id, dto, reviewedBy), 'VIP upgrade request reviewed'); }
}
