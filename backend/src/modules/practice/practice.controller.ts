...
export class PracticeQuestionController {
  ...
  @Get() async findAll(@Query() q: PracticeQuestionQueryDto) { return ok(await this.svc.findAll(q), 'Practice questions retrieved'); }
  @Get(':id') async findOne(@Param('id') id: string) { return ok(await this.svc.findById(id), 'Practice question retrieved'); }
  @Post() @HttpCode(HttpStatus.CREATED) async create(@Body() dto: CreatePracticeQuestionDto) { return ok(await this.svc.create(dto), 'Practice question created'); }
...
export class PracticeAttemptController {
  ...
  @Get() async findAll(@Query() q: PracticeAttemptQueryDto) { return ok(await this.svc.findAll(q), 'Practice attempts retrieved'); }
  @Get(':id') async findOne(@Param('id') id: string) { return ok(await this.svc.findById(id), 'Practice attempt retrieved'); }
  @Post() @HttpCode(HttpStatus.CREATED) async start(@Body() dto: StartPracticeAttemptDto, @CurrentUser('sub') userId: string) { return ok(await this.svc.start(dto, userId), 'Practice attempt started'); }
  @Patch(':id') async submit(@Param('id') id: string, @Body() dto: SubmitPracticeAttemptDto) { return ok(await this.svc.submit(id, dto), 'Practice attempt submitted'); }
}
