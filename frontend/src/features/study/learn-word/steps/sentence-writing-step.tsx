...
export function SentenceWritingStep({ vocabulary, onNext }: SentenceWritingStepProps) {
  ...
  return (
    <div className="flex flex-col min-h-full w-full max-w-3xl mx-auto items-center animate-in fade-in slide-in-from-bottom-8 duration-700 relative">
      ...
      {!isPassed ? (
        <div className="w-full max-w-sm flex flex-col items-center gap-2 mt-auto">
          <button 
            onClick={handleCheck}
            disabled={!sentence.trim() || isChecking}
            className="w-full bg-gray-900 hover:bg-black text-white py-3.5 text-lg rounded-2xl shadow-[0_10px_25px_-5px_rgba(0,0,0,0.3)] hover:shadow-[0_15px_30px_-5px_rgba(0,0,0,0.4)] hover:-translate-y-0.5 transition-all font-bold flex items-center justify-center gap-2 disabled:opacity-40 disabled:hover:translate-y-0 disabled:hover:shadow-none cursor-pointer"
          >
          ...
          >
            Bỏ qua bước đặt câu này →
          </button>
        </div>
      ) : (
        <button 
          onClick={onNext}
          className="w-full max-w-sm bg-[#1f5333] hover:bg-[#163f26] text-white py-3.5 text-lg rounded-2xl shadow-[0_10px_25px_-5px_rgba(31,83,51,0.4)] hover:shadow-[0_15px_30px_-5px_rgba(31,83,51,0.6)] hover:-translate-y-0.5 transition-all font-bold flex items-center justify-center gap-2 mt-auto animate-in fade-in duration-500 cursor-pointer"
        >
