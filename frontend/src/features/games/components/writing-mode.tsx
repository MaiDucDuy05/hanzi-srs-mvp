...
/**
 * Component for Animation Preview of the Character
 */
function HanziWriterAnimation({ char, speed }: { char: string; speed: 'slow' | 'normal' | 'fast' }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const playAnimation = () => {
        ...
        try {
          writer.animateCharacter({
            ...
            onComplete: () => {
              animationTimeout = setTimeout(playAnimation, 2000);
            }
          });
        } catch (e) {
      ...
      // Wait for char data to load before animating
...
export function WritingMode({ items, initialState, onStateChange, onComplete }: WritingModeProps) {
  ...
  const [key, setKey] = useState(0); // for forcing re-render of canvas (e.g. Clear)
  ...
  const handleComplete = (mistakes: number = 0) => {
    secRef.current?.complete(mistakes);
  };
    ...
    <div className="w-full min-h-[calc(100vh-4rem)] bg-[#f9f8f4] text-[#333] font-sans flex flex-col items-center py-10 px-4 relative overflow-hidden rounded-xl border border-[#e8e2d2]">
      ...
      <div className="max-w-4xl w-full relative z-10">
        ...
        <div className="flex items-center justify-between border-b-2 border-[#d0c9b7] pb-3 mb-8">
          ...
          <div className="text-sm font-semibold text-[#8b7e66]">
            Tiến độ: {ctx.charIndex + 1}/{ctx.totalChars}
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-10 justify-center items-start mb-8">
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="mt-5 flex gap-3 w-[350px]">
              <button 
                onClick={clearCanvas}
                className="flex-1 py-2 text-sm bg-white border border-[#d0c9b7] rounded font-semibold text-[#5a5038] hover:bg-[#f0ebe1] transition-colors shadow-sm"
              >
                Viết lại nét sai
              </button>
              <button 
                onClick={handleSkip}
                className="flex-1 py-2 text-sm bg-[#e8e2d2] border border-[#a3977c] rounded font-semibold text-[#4a422e] hover:bg-[#d6ceb8] transition-colors shadow-sm"
              >
                Bỏ qua chữ này
              </button>
            </div>
            ...
            {ctx.feedback === 'done' && (
              <p className="mt-3 font-medium text-[#4caf50]">Bạn đã viết đúng! Đang chuyển chữ...</p>
            )}
          </div>
          ...
          <div className="bg-[#f3efdf] border-2 border-[#d0c9b7] p-5 rounded-lg shadow-sm w-[280px] flex flex-col items-center relative">
            ...
            <div className="relative bg-white border-2 border-[#a3977c] w-[180px] h-[180px] mb-5">
              ...
              {ctx.currentHanzi && (
                <HanziWriterAnimation char={ctx.currentHanzi} speed={speed} />
              )}
            </div>
