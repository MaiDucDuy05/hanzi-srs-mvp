...
export function Balloon({ target }: BalloonProps) {
  ...
  return (
    <div
      className={cn('absolute flex flex-col items-center transition-all duration-100', target.fullyTyped && 'animate-pulse')}
      style={{
        left: `${target.x}%`, top: `${target.y}%`,
        transform: 'translate(-50%, -50%)',
        filter: target.fullyTyped ? 'drop-shadow(0 0 8px rgba(0,200,83,0.6))' : 'drop-shadow(0 4px 6px rgba(0,0,0,0.15))',
      }}
    >
      <div className={cn('relative w-16 h-20 rounded-t-full rounded-b-2xl flex items-center justify-center shadow-lg bg-gradient-to-b from-white to-90% border-2', colors.bg, colors.border)}>
        <span className="text-3xl font-black text-white drop-shadow-md">{target.hanzi}</span>
      ...
      <div className="mt-1 text-xs text-emerald-700/60 font-semibold bg-white/40 px-2 py-0.5 rounded-full">{target.pinyinDisplay}</div>
    </div>
  );
}
