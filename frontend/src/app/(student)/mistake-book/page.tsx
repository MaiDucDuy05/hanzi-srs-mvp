...
export default function MistakeBookPage() {
  ...
  useEffect(() => {
    curriculumApi
      .listVocabularies({ status: 'PUBLISHED' })
      .then(setVocab)
      .catch(() => setVocab([]));
  }, []);
      ...
      <div className="space-y-6">
        <header>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Ghi lại những từ hay nhầm để ôn tập lại sau này.
          </p>
        </header>
        ...
        <Card>
          ...
          <CardBody>
            <form onSubmit={add} className="flex flex-col gap-3 sm:flex-row sm:items-end">
              ...
              <Button type="submit" disabled={!selectedId} loading={saving}>
                Thêm
              </Button>
            </form>
          </CardBody>
        ...
        {!loading && !error && (
          <div className="space-y-2">
            ...
            {entries.length === 0 && (
              <p className="text-sm text-gray-500">Sổ tay còn trống — thêm từ đầu tiên nhé!</p>
            )}
          </div>
        )}
      </div>
