...
const MOCK_DATA: Record<string, { id: string; title: string; count: number; desc: string }[]> = {
  hsk: [
    { id: 'hsk1', title: 'HSK 1', count: 150, desc: 'Basic vocabulary for beginners' },
    { id: 'hsk2', title: 'HSK 2', count: 300, desc: 'Elementary vocabulary' },
    { id: 'hsk3', title: 'HSK 3', count: 600, desc: 'Intermediate vocabulary' },
    { id: 'hsk4', title: 'HSK 4', count: 1200, desc: 'Advanced vocabulary' },
  ],
  topic: [
    { id: 'food', title: 'Food & Drinks', count: 45, desc: 'Words related to eating' },
    { id: 'travel', title: 'Travel', count: 30, desc: 'Words for getting around' },
    { id: 'family', title: 'Family', count: 20, desc: 'Family members and relatives' },
    { id: 'animals', title: 'Animals', count: 25, desc: 'Common animals' },
  ],
  assignment: [
    { id: 'hw1', title: 'Homework 1', count: 10, desc: 'Assigned on Oct 10' },
    { id: 'hw2', title: 'Homework 2', count: 15, desc: 'Assigned on Oct 12' },
  ],
  mistakes: [
    { id: 'recent', title: 'Recent Mistakes', count: 20, desc: 'Words you got wrong this week' },
    { id: 'all', title: 'All Mistakes', count: 85, desc: 'Complete list of all mistakes' },
  ]
};

const MODE_TITLES: Record<string, string> = {
  hsk: 'Select HSK Level',
  topic: 'Select a Topic',
  assignment: "Teacher's Assignments",
...
export default function LessonSelectionPage() {
  ...
  const handleLessonClick = (lessonId: string, lessonTitle: string) => {
    if (mode === 'assignment') {
      // Direct pass for assignments (assume teacher setup Flashcard for demo)
      router.push(`/games/flashcard?mode=assignment&lesson=${lessonId}`);
    } else {
      // Open modal to select game
  ...
  return (
    <div className="w-full flex flex-col min-h-full py-4 sm:py-0 relative">
      ...
      <div className="flex items-center gap-4 mb-8 pl-2">
        <button 
          onClick={() => router.back()}
          className="w-12 h-12 flex-shrink-0 flex items-center justify-center rounded-full bg-white shadow-sm hover:bg-gray-50 text-[#215b3b] transition-colors border-2 border-transparent hover:border-[#aadd4a]"
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 19l-7-7 7-7" />
          </svg>
        </button>
        <h1 className="text-3xl sm:text-4xl font-black text-[#3e5c46] font-heading tracking-tight">
          {title}
        </h1>
      </div>
        ...
        {lessons.length > 0 ? (
          lessons.map((lesson) => (
            <button 
              ...
              className="text-left bg-white rounded-[2rem] p-6 sm:p-8 shadow-sm border-4 border-transparent hover:border-[#aadd4a] hover:shadow-md transition-all group flex items-center justify-between"
            >
              <div>
                <h2 className="text-2xl font-bold text-[#215b3b] mb-1 group-hover:text-[#4a6b38] transition-colors">
                  {lesson.title}
                </h2>
          ...
          <div className="col-span-1 md:col-span-2 text-center py-20 text-gray-500 font-medium text-xl">
            No lessons available for this mode yet.
          </div>
        )}
      </div>
      ...
      {selectedLesson && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setSelectedLesson(null)}></div>
          <div className="relative bg-white w-full max-w-3xl rounded-[2.5rem] shadow-2xl p-6 sm:p-10 animate-in fade-in zoom-in duration-200">
            ...
            <div className="text-center mb-8">
              <h2 className="text-3xl font-black text-[#215b3b] font-heading">
                Practice <span className="text-[#8bc34a]">"{selectedLesson.title}"</span>
              </h2>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {GAMES.map((game) => (
                <button 
                  ...
                  className="bg-white rounded-[2rem] p-6 shadow-sm border-4 border-transparent hover:border-[#aadd4a] bg-[#f9fdf5] hover:bg-white transition-all hover:shadow-md flex flex-col items-center justify-center text-center relative group"
                >
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <DecorativeLeaves />
                  </div>
                  <div className="h-20 flex items-center justify-center transform group-hover:scale-110 transition-transform duration-300">
                    {game.icon}
                  </div>
                  <h3 className="text-lg font-bold text-[#111] mt-2 group-hover:text-[#4a6b38] transition-colors">
                    {game.title}
                  </h3>
                </button>
              ))}
